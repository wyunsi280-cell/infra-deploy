...
```bash
curl -fsSL https://raw.githubusercontent.com/wyunsi280-cell/infra-deploy/main/devpi/install.sh -o /tmp/devpi-install.sh
...
用法(发布/消费私有库)见 [devpi/README.md](./devpi/README.md) 或直接看 [`install.sh`](./devpi/install.sh) 跑完之后打印的说明。

## 已知限制

- 现在只在内网/WSL 里验证过,还没做公网暴露(Cloudflare 那一层),等真正需要外部访问的时候再加
